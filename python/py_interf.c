...
static PyObject *fastdp_py(PyObject *self, PyObject *args, PyObject *kwargs) {
  import_array();
  PyArrayObject *py_v;
  int knnk, numclu, w = 0, maxiter = 100;
  float nndes = 0.0, delta = 0.05;
  char *type = NULL;
  char *distance = NULL;

  PyObject *ret;
  static char *kwlist[] = {"v",       "numclu", "knnk",  "window",   "nndes",
                           "maxiter", "delta",  "dtype", "distance", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O!i|iififss", kwlist, &PyArray_Type, &py_v,
                                   &numclu, &knnk, &w, &nndes, &maxiter, &delta, &type,
                                   &distance)) {
    return NULL;
  }
  
  // DataSet *DS = init_DataSet(N, D);
  py_densityPeaks(py_v,15);
